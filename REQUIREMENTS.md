...
2. **The DOM overlay.** Sits on top of / alongside the canvas. Handles:
   ...
   - Text sections (milestone write-ups) interspersed between visual
     sections.
   - The tap-to-expand drawer (see below).
...
Tapping/clicking a month card:
- Pauses the main scroll.
- Opens a full-screen mobile drawer (bottom sheet) with a shared-element
  transition from the tapped card's image into the drawer's header image.
- The drawer shows: the rest of that month's photos (grid/masonry), any
  specific-date milestones within the month, captions, and freeform notes.
- Closing the drawer resumes the main scroll where it left off.
...
| Shared-element / drawer motion | `motion` (not `framer-motion` — renamed, same API) | Import from `motion/react`. |
...
1. **Resize/compress every source photo before it becomes a texture.**
   ...
   on a phone screen and burns GPU memory.
...
- Full photo sets for each month's drawer.
